(function(win){
  ...
  function write_result_to_table(table, row_index, data) {
    ...
    var template = Handlebars.compile(source);
	var video_url = data.video_url.replace("&feature=youtube_gdata", "");
    var context = {index: row_index, padded_index: pad(row_index, 2), url: video_url, video_title: data.video_title};
